...
def hash_nn(r):
    ...
    l4 = tf.nn.leaky_relu(tf.matmul(l3, H_W4) + H_B4, .2)
    l5 = tf.nn.leaky_relu(tf.matmul(l4, H_W5) + H_B5, .2)
    out = tf.nn.tanh(tf.matmul(l5, H_W6) + H_B6)
    return out
...
if __name__ == '__main__':
    ...
    # Layer 2 Variables
    H_W2 = tf.Variable(xavier_init([1024, 2048]))
    H_B2 = tf.Variable(xavier_init([2048]))

    # Layer 3 Variables
    H_W3 = tf.Variable(xavier_init([2048, 2048]))
    H_B3 = tf.Variable(xavier_init([2048]))

    # Layer 4 Variables
    H_W4 = tf.Variable(xavier_init([2048, 512]))
    H_B4 = tf.Variable(xavier_init([512]))

    # Layer 5 Variables
    H_W5 = tf.Variable(xavier_init([512, 128]))
    H_B5 = tf.Variable(xavier_init([128]))

    # Layer 6 Variables
    H_W6 = tf.Variable(xavier_init([128, 16]))
    H_B6 = tf.Variable(xavier_init([16]))

    # Store Variables in list
    H_var_list = [H_W1, H_B1, H_W2, H_B2, H_W3, H_B3, H_W4, H_B4, H_W5, H_B5, H_W6, H_B6]
